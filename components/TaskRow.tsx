...
import { RewordTaskDialog } from "@/components/RewordTaskDialog";
...
const TaskRow = ({ task, onDelete, onToggleComplete, onReword, onAcceptReword }: TaskRowProps) => {
  const [isRewordDialogOpen, setIsRewordDialogOpen] = useState(false);
  ...
  return (
    <>
      <TableRow className="hover:bg-muted/50">
        ...
        <TableCell className="text-right py-2">
          <Button
            ...
            size="icon"
            className="h-8 w-8"
            onClick={() => setIsRewordDialogOpen(true)}
            title="Reword with AI"
          >
            <Sparkles className="h-4 w-4" />
            <span className="sr-only">Reword with AI</span>
          </Button>
      ...
      <RewordTaskDialog
        task={task}
        open={isRewordDialogOpen}
        onOpenChange={setIsRewordDialogOpen}
        onReword={onReword}
        onAccept={onAcceptReword}
      />
    </>
  );
};
